...
def clone(src, dst):
    if (
        subprocess.run(["cp", "-c", str(src), str(dst)], capture_output=True).returncode
        != 0
    ):
...
def main():
    ...
    ap.add_argument(
        "--out", required=True, type=Path, help="output directory for the folded shards"
    )
