...
enum LTXModelCatalog {
    ...
    static let all: [LTXModel] = [
        ...
        LTXModel(
            ...
            downloadSize: "~100GB + ~8GB LoRA",
            supportsBuiltInAudio: true,
            qualityWarning: "Official production path: half-res dev+CFG, upscale, fuse distilled LoRA into the dev DiT. First Dev generate downloads ltx-2.5-22b-distilled-lora-450-bf16 (~8.3GB) from dgrauet/ltx-2.5-mlx (gated — accept that repo as the same HF account as hf auth login). Much slower. 64GB+; 128GB comfortable.",
            recommendedStepsLower: 20,
            recommendedStepsUpper: 40,
            tips: "Uses transformer-dev.safetensors + distilled LoRA from dgrauet/ltx-2.5-mlx for Stage 2. Slider is stage-1 steps (default 30). Stage-2 is 3. CFG default 3. Requires ltx-2-mlx 0.15+.",
            backend: .ltx2Mlx,
            minRecommendedRAMGB: 64,
            usesBundledGemma4: true,
